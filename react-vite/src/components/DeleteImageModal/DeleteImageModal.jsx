...
function DeleteImageModal({ imageId, onlyImage, businessId }) {
    ...
    const handleDelete = (e) => {
        ...
        const runDispatches = async () => {
            await dispatch(deleteImage(imageId)).then(() => dispatch(getUserImages(user.id))).then(() => dispatch(getImagesByBusiness(businessId))).then(() => dispatch(fetchSingleBusiness(businessId)).then(closeModal)
            );
        ...
        <>
            {user && !onlyImage ? (<div className="deleteModal">
                ...
                (user && <div className="deleteModal">
                    <h1>Cannot Delete</h1>
                    <span>As the business owner, you must have at least one photo uploaded. Please add another photo before deleting.</span>
                </div>)
            }
            {
                !user &&
